...
# settingsFile="json/translate/example.json"
cleanCompile=false
noCompile=false
override=false
verbose=false
...
################################################
# Run this app
translate_cmd="$BUILD_DIR/src/app/translate_tpstream -j $settingsFile" 
# optional overrides
if [ ! -z ${inputFile+x} ]; then
  translate_cmd="$translate_cmd --input-file $inputFile"
fi
if [ ! -z ${output_folder+x} ]; then
  translate_cmd="$translate_cmd --output-folder $output_folder"
fi
if [ ! -z "$max_files" ]; then
  translate_cmd="$translate_cmd --max-files $max_files"
fi
if [ ! -z "$skip_files" ]; then
  translate_cmd="$translate_cmd --skip-files $skip_files"
fi
if [ "$override" = true ]; then
  translate_cmd="$translate_cmd --override"
fi
if [ "$verbose" = true ]; then
  translate_cmd="$translate_cmd -v"
fi
if [ "$debug" = true ]; then
  translate_cmd="$translate_cmd -d"
fi
echo "Running: $translate_cmd"
exec $translate_cmd
