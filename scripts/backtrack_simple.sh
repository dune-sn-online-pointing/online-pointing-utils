...
################################################
# Run this app
backtrack_simple_cmd="$BUILD_DIR/src/app/backtrack_tpstream_simple -j $settingsFile" 
# optional overrides
if [ ! -z ${inputFile+x} ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd --input-file $inputFile"
fi
if [ ! -z ${output_folder+x} ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd --output-folder $output_folder"
fi
if [ ! -z "$max_files" ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd --max-files $max_files"
fi
if [ ! -z "$skip_files" ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd --skip-files $skip_files"
fi
if [ "$override" = true ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd --override"
fi
if [ "$verbose" = true ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd -v"
fi
if [ "$debug" = true ]; then
  backtrack_simple_cmd="$backtrack_simple_cmd -d"
fi
echo "Running: $backtrack_simple_cmd"
exec $backtrack_simple_cmd
