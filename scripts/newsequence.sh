...
backtrack_command="${HOME_DIR}/scripts/backtrack_simple.sh $common_options"
...
add_backgrounds_command="${HOME_DIR}/scripts/add_backgrounds_simple.sh $common_options"
if [ "$run_add_backgrounds" = true ] && [ "$clean_clusters" = false ]; then
        echo "Running add backgrounds Simple step with command:"
        echo $add_backgrounds_command
        $add_backgrounds_command
        if [ $? -ne 0 ]; then
                echo "Error: Add backgrounds Simple step failed."
                exit 1
        fi
        echo ""
fi
...
make_clusters_command="${HOME_DIR}/scripts/make_clusters_simple.sh $common_options"
