...
bool superduperdebugMode = false;
...
//             tpTree->GetEntry(i); 
//             if (verboseMode){
//                 LogInfo << " cluster read tps " << i << " event " << event << " run " << run << std::endl;
//             }
//             if (event != theevent) continue;
...
std::vector<Cluster> make_cluster(std::vector<TriggerPrimitive*> all_tps, int ticks_limit, int channel_limit, int min_tps_to_cluster, int adc_integral_cut) {
    ...
    for (int iTP = 0; iTP < all_tps.size(); iTP++) {
        ...
        if (superduperdebugMode) LogInfo << "Processing TP: " << tp1->GetTimeStart() << " " << tp1->GetDetectorChannel() << std::endl;
        ...
        for (auto& candidate : buffer) {
            ...
            if (can_append) {
                ...
                appended = true;
                if (superduperdebugMode) LogInfo << "Appended TP to candidate Cluster" << std::endl;
                break;
            }
        }
        ...
        if (!appended) {
            if (superduperdebugMode) LogInfo << "Creating new candidate Cluster" << std::endl;
...
void write_clusters(std::vector<Cluster>& clusters, TFile* clusters_file, std::string view) {
    ...
    if (!clusters_tree) {
        ...
        clusters_tree->Branch("run", &run, "event/I");
        ...
        clusters_tree->SetBranchAddress("run", &run);
    ...
    // fill the tree
    int count = -1;
    if (verboseMode) LogInfo << " total clusters found " << clusters.size() << std::endl;
    for (auto& Cluster : clusters) {
        count ++;
        if (verboseMode) LogInfo << "Cluster size "<< Cluster.get_size() << std::endl;
        if (Cluster.get_size() <= 0){
            LogError << " Cluster size is zero for cluster " << count << std::endl;
            continue;
        }
        ...
        if (verboseMode) LogInfo << "Clusters: " << n_tps << " event: " << event << " run: " << run << std::endl;
    ...
    if (tp_samples_to_peak) tp_samples_to_peak->clear();
        ...
        if (verboseMode) LogInfo << "clusters_tree->Fill" << Cluster.get_tps().size() << std::endl;
        clusters_tree->Fill();
    }
...
void write_clusters_with_match_id(std::vector<Cluster>& clusters, std::map<int, int>& cluster_to_match, TFile* clusters_file, std::string view,
    ...
    clusters_tree->Branch("event", &event, "event/I");
    clusters_tree->Branch("run", &event, "run/I");
    clusters_tree->Branch("n_tps", &n_tps, "n_tps/I");
    clusters_tree->Branch("true_pos_x", &true_pos_x, "true_pos_x/F");
    clusters_tree->Branch("true_pos_y", &true_pos_y, "true_pos_y/F");
    clusters_tree->Branch("true_pos_z", &true_pos_z, "true_pos_z/F");
    ...
    // Fill the tree
    for (auto& Cluster : clusters) {
        event = (int)Cluster.get_event();
        run = (int)Cluster.get_run();
        n_tps = Cluster.get_size();
        true_pos_x = Cluster.get_true_pos()[0];
        true_pos_y = Cluster.get_true_pos()[1];
        true_pos_z = Cluster.get_true_pos()[2];
        ...
        if (debugMode) LogInfo << " fill cluster_tree " << std::endl;
        clusters_tree->Fill();
    }
