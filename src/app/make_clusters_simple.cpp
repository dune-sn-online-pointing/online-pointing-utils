...
int main(int argc, char *argv[])
{
    ...
    for (const auto &tps_file : inputs){
        ...
        std::vector<TrueParticle> true_by_event;
        std::vector<Neutrino> nu_by_event;
        std::map<const ULong_t, UInt_t> tp_map_lo;
        std::map<const ULong_t, UInt_t> tp_map_hi;

        TFile inFile(tps_file.c_str(), "READ");
        if (inFile.IsZombie())
        {
            LogError << "Cannot open: " << tps_file << std::endl;
            continue;
        }
        ...
        if (auto *tpTree = dynamic_cast<TTree *>(inFile.Get("tps")))
        {
            ...
            int event_count = generate_filemap(tpTree, tp_map_lo, tp_map_hi);
            ...
            // Process events
            int count = 0;
            for (auto index : tp_map_lo){
                ULong_t key = index.first;
                UInt_t lo = index.second;
                UInt_t hi = tp_map_hi[key];
                UInt_t run = run_from_event_key(key);
                UInt_t event = event_from_event_key(key);
                tps_by_event.clear();
                tps_by_event.reserve(hi-lo+1);
                if (debugMode){
                    std::printf("new tps event pointers: count: %d, index: %ld, event: %d, run %d\n", count, index.first, event, run);
                }
                count++;
                read_tps_simple(tpTree, event, run, lo, hi, tps_by_event);

                // Apply ToT cut to TPs if requested
                if (tot_cut > 0)
                {

                    auto &vec = tps_by_event;
                    vec.erase(
                ...
                auto &tps = tps_by_event;
                if (verboseMode && count < 10){
                    for (auto tp:tps_by_event){
                        LogInfo << " DUMP A TP" << std::endl;
                        tp.Print();
                    }
                }
                ...
                for (size_t iView = 0; iView < APA::views.size(); ++iView)
                {
                    ...
                    {
                        if (verboseMode) LogInfo << "cluster label" << cluster.get_true_label() << std::endl;
                ...
                for (size_t iView = 0; iView < APA::views.size(); ++iView)
                {
                    ...
                    for (auto &cluster : clusters_per_view.at(iView))
                    {
                        ...
                        if(debugMode) cluster.printClusterInfo();
