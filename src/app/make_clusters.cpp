...
int main(int argc, char *argv[])
{
    CmdLineParser clp;
    clp.getDescription()
        << "> Cluster app - build clusters from *_tps.root files."
        << std::endl;
    clp.addDummyOption("Main options");
    clp.addOption("json", {"-j", "--json"},
                  "JSON file containing the configuration");
    clp.addOption("inputFile", {"-i", "--input-file"},
                  "Input file with list OR single ROOT file path "
                  "(overrides JSON inputs)");
    clp.addOption("skip_files", {"-s", "--skip", "--skip-files"},
                  "Number of files to skip at start (overrides JSON)",
                  -1);
    clp.addOption("max_files", {"-m", "--max", "--max-files"},
                  "Maximum number of files to process (overrides JSON)",
                  -1);
    clp.addOption("apa", {"-a", "--apa", "--apa-filter"},
                  "Filter TPs by APA index (e.g. 1 for APA1). "
                  "Use -1 to disable.",
                  -1);
    clp.addOption("override", {"-f", "--override"},
                  "Override existing output files (default: false)",
                  false);
    clp.addOption("outFolder", {"--output-folder"},
                  "Output folder path (default: data)");
    clp.addTriggerOption("verboseMode", {"-v"},
                         "RunVerboseMode, bool");
    clp.addTriggerOption("debugMode", {"-d"},
                         "Run in debug mode (more detailed than verbose)");
    clp.addDummyOption();
    ...
    if (clp.isOptionTriggered("verboseMode"))
    {
        verboseMode = true;
    }
    ...
    // Use tpstream-based file tracking for consistent skip/max
    // across pipeline. By default, load tps_bg files (TPs with
    // backgrounds merged)
    std::vector<std::string> inputs =
        find_input_files_by_tpstream_basenames(j, "tps_bg", skip_files,
                                               max_files);
    ...
    LogInfo << "Found " << inputs.size()
            << " files with backgrounds (tps_bg)" << std::endl;
    LogThrowIf(inputs.empty(),
               "No tps_bg files found. Please run add_backgrounds step "
               "first to merge signal and background TPs.");
    ...
    if (!inputs.empty())
    {
        ...
        std::string input_folder =
            (last_slash != std::string::npos) ? first_file.substr(0, last_slash) : ".";
    ...
    float conversion_col =
        ParametersManager::getInstance().getDouble(
            "conversion.adc_to_energy_factor_collection");
    float adc_integral_cut_col = energy_cut * conversion_col;
    float conversion_ind =
        ParametersManager::getInstance().getDouble(
            "conversion.adc_to_energy_factor_induction");
    float adc_integral_cut_ind = energy_cut * conversion_ind;
    ...
    else
    {
        ...
        clusters_folder_path =
            getOutputFolder(j, "clusters", "clusters_folder");
    }

    LogInfo << "Settings from json file:" << std::endl;
    LogInfo << " - Clusters output path: "
            << clusters_folder_path << std::endl;
    LogInfo << " - Tick limit: " << tick_limit << std::endl;
    LogInfo << " - Channel limit: " << channel_limit << std::endl;
    LogInfo << " - Minimum TPs to form a cluster: "
            << min_tps_to_cluster << std::endl;
    LogInfo << " - Energy cut: " << energy_cut << std::endl;
    LogInfo << "    - ADC integral cut (induction): "
            << adc_integral_cut_ind << std::endl;
    LogInfo << "    - ADC integral cut (collection): "
            << adc_integral_cut_col << std::endl;
    LogInfo << " - ToT cut: " << tot_cut << std::endl;
    LogInfo << " - APA filter: "
            << (apa_filter >= 0 ? std::to_string(apa_filter) : std::string("disabled")) << std::endl;
    LogInfo << " - Files to process (after skip/max): "
    ...
    auto create_metadata_tree = [&](TFile *file)
    {
        file->cd();
        TTree *metadata_tree =
            new TTree("clustering_metadata", "Clustering parameters used");
        ...
        float meta_adc_to_mev_collection =
            ParametersManager::getInstance().getDouble(
                "conversion.adc_to_energy_factor_collection");
        float meta_adc_to_mev_induction =
            ParametersManager::getInstance().getDouble(
                "conversion.adc_to_energy_factor_induction");
        ...
        metadata_tree->Branch("adc_integral_cut_collection", &meta_adc_cut_col,
                              "adc_integral_cut_collection/I");
    ...
    for (const auto &tps_file : inputs)
    {
        ...
        {
            LogWarning
                << "File doesn't match expected *_tps.root pattern: " << tps_file << std::endl;
            continue;
        }
        ...
        if (verboseMode)
            LogInfo << "Output clusters file: " << current_clusters_filename << std::endl;
        ...
        if (apa_filter >= 0)
        {
            for (auto &kv : tps_by_event)
            {
                auto &vec = kv.second;
                vec.erase(
                    std::remove_if(vec.begin(), vec.end(), [&](const TriggerPrimitive &tp)
                                   { return tp.GetDetector() != apa_filter; }),
                    vec.end());
            }
        }
        ...
        if (tot_cut > 0)
        {
            for (auto &kv : tps_by_event)
            {
                ...
                vec.erase(
                    std::remove_if(vec.begin(), vec.end(), [&](const TriggerPrimitive &tp)
                                   { return (int)tp.GetSamplesOverThreshold() <= tot_cut; }),
                    vec.end());
            }
        }
        ...
        for (auto &kv : tps_by_event)
        {
            ...
            for (size_t iView = 0; iView < APA::views.size(); ++iView)
            {
                clusters_per_view.emplace_back(
                    make_cluster(tps_per_view.at(iView),
                                 tick_limit,
                                 channel_limit,
                                 min_tps_to_cluster,
                                 adc_cut.at(iView)));
            ...
            for (size_t iView = 0; iView < APA::views.size(); ++iView)
            {
                ...
                {
                    if (cluster.get_true_label() != "marley")
                        continue; // only consider marley clusters
