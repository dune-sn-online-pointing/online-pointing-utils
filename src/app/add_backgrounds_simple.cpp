...
int main(int argc, char* argv[]) {
    ...
    LogInfo << "output folder is " << output_folder << std::endl;
    ...
    for (auto bkg_file:bkg_files){
        LogInfo << " Background file " << bkg_file << std::endl;
    }
    ...
    std::vector<TriggerPrimitive> current_bkg_tps;
    // std::vector<TrueParticle> current_bkg_true;
    // std::vector<Neutrino> current_bkg_nu;
    std::vector<ULong_t> bk_current_event_ids;
    std::map<const ULong_t, UInt_t> bk_map_lo;
    std::map<const ULong_t, UInt_t> bk_map_hi;
    std::string bkg_file = bkg_files[bkg_file_idx];

    TFile bkgFile(bkg_file.c_str(), "READ");

    // Load first background file
    //read_tps_simple(bkg_files[bkg_file_idx], current_bkg_tps, current_bkg_true, current_bkg_nu);
    
    TTree *bkTree = dynamic_cast<TTree *>(bkgFile.Get("tps"));
    int bk_event_count = generate_filemap(bkTree, bk_map_lo, bk_map_hi);

    LogInfo << "Bkg Tree " << bkTree->GetName() << " has " << bk_event_count << "entries" << std::endl;
    for (const auto& kv : bk_map_lo) {
        bk_current_event_ids.push_back(kv.first);
        if(debugMode) LogInfo << "add bgk event" << kv.first << std::endl;
    }
    ...
    auto get_next_bkg_event = [&]() -> std::tuple<int, std::vector<TriggerPrimitive>> {
        ...
        if (debugMode) LogInfo << " getting next bkg event " << std::endl;
        current_bkg_tps.clear();
        if (bkg_event_idx >= bk_current_event_ids.size()) {
            LogInfo << " get next background event from a new file" << std::endl;
            bkg_event_idx = 0;
            ...
            //bk_current_event_ids.clear();
            LogInfo << " looking in new background file " << std::endl;
            bkgFile.Close();
            ...
            bkg_file = bkg_files[bkg_file_idx];

            bkgFile.Open(bkg_file.c_str(), "READ");

            // Load first background file
            //read_tps_simple(bkg_files[bkg_file_idx], current_bkg_tps, current_bkg_true, current_bkg_nu);
    
            bkTree = dynamic_cast<TTree *>(bkgFile.Get("tps"));
        ...
        ULong_t bg_event_id = bk_current_event_ids[bkg_event_idx];
        UInt_t bg_lo = bk_map_lo[bg_event_id];
        UInt_t bg_hi = bk_map_hi[bg_event_id];
        //if (debugMode) bg_hi = bg_lo + 5;
        UInt_t bg_run = run_from_event_key(bg_event_id); //(UInt_t)(bg_event_id / 10000000);
        UInt_t bg_event = event_from_event_key(bg_event_id); // (UInt_t)(bg_event_id % 10000000);
        assert(bg_run < 40000000);
        if (debugMode) LogInfo << "check bkg id " << bg_event_id << " " << bg_event << " " << bg_run << std::endl;
        current_bkg_tps.clear();
        read_tps_simple(bkTree, bg_event, bg_run, bg_lo, bg_hi, current_bkg_tps);
        if (debugMode) LogInfo << "BKG TPS read" << bg_event << " lo  " << bg_lo << " hi " << bg_hi << " count " << current_bkg_tps.size() << std::endl;;
        bool test = SingleEventChecker(current_bkg_tps);
        if (!test){LogError << "bad bkg event read in" << std::endl;}
        auto & bkg_tps = current_bkg_tps;
        
        if (verboseMode) {
            LogInfo << "Using background: file " << std::filesystem::path(bkg_files[bkg_file_idx]).filename().string()
                    << " bg_event " << bg_event_id << " (file " << (bkg_file_idx + 1) << "/" << bkg_files.size()
                    << ", bkg event  " << (bkg_event_idx + 1) << "/" << bk_current_event_ids.size() << ")" << std::endl;
        }
        
        bkg_event_idx++;
        return {bg_event_id, bkg_tps};
    };
    ...
    for (const auto& signal_file : signal_files) {
        ...
        //TFile* output_file_pointer;
        

        //TTree* output_tps_tree = open_tps_simple(signal_file, output_file_pointer);
        // Read signal TPs
        // if (!output_file_pointer){
        //     LogError << " Signal output file pointer is 0" << std::endl;
        // }
        // if (!output_tps_tree){
        //     LogError << " Signal tree pointer is 0" << std::endl;
        // }
        std::vector<TriggerPrimitive> signal_tps_by_event;
        std::vector<TrueParticle> signal_true_by_event;
        std::vector<Neutrino> signal_nu_by_event;
        std::map<const ULong_t, UInt_t> sg_map_lo;
        std::map<const ULong_t, UInt_t> sg_map_hi;
        ...
        tps_writer.Create(output_filename, "tps");
        ...
        TFile sigFile(signal_file.c_str(), "READ");
        
        TTree *sigTree = dynamic_cast<TTree *>(sigFile.Get("tps"));
        ...
        // std::vector<TriggerPrimitive*> merged_tps;
        LogInfo << " make signal map " << std::endl;
        int sig_count = generate_filemap(sigTree, sg_map_lo, sg_map_hi);
        LogInfo << "Signal Tree " << sigTree->GetName() << " has " << sig_count << "entries" << std::endl;
        LogInfo << "sigFile.GetName()" << sigFile.GetName() << std::endl;
        // Track signal TP counts for each event (for diagnostics)
        std::vector<int> signal_tp_counts;
        LogInfo << " signal count for " << signal_file << " is " << sig_count;
        
        // Process each event in the signal file
        int total_tps_count = 0;
         
        for (const auto& kv : sg_map_lo) {
            ULong_t sg_event_id = kv.first;
            if (total_tps_count == 0) {
                LogInfo << " look in sg_map_lo " << kv.first << " " << kv.second << std::endl;
            }
            
            UInt_t sg_lo = sg_map_lo[sg_event_id];
            UInt_t sg_hi = sg_map_hi[sg_event_id];
            UInt_t sg_run = run_from_event_key(sg_event_id); //(UInt_t)(sg_event_id / 10000000);
            UInt_t sg_event = event_from_event_key(sg_event_id); // (UInt_t)(sg_event_id % 10000000);
            
            int tpscount = sg_hi - sg_lo + 1;
            if (debugMode) LogInfo << " look for signal event " << sg_event_id << std::endl;
            //signal_tps_by_event.clear();
            //signal_tps_by_event.reserve(tpscount);
            if (debugMode) LogInfo << "check sig id " << sg_event_id << " " << sg_event << " " << sg_run << std::endl;
            signal_tps_by_event.clear();
            read_tps_simple(sigTree, sg_event, sg_run, sg_lo, sg_hi, signal_tps_by_event);
            bool sg_test = SingleEventChecker(signal_tps_by_event);
            if (!sg_test) LogError << "Inconsistent signal event" << sg_event_id << std::endl;
            const auto& signal_tps = signal_tps_by_event;
            // Get neutrino vertex position if around_vertex_only is enabled
            TVector3 vertex_pos(0, 0, 0);
            bool has_vertex = false;
            if (around_vertex_only && tpscount > 0) {
                const auto& neutrinos = signal_nu_by_event;
                if (!neutrinos.empty()) {
                    vertex_pos.SetXYZ(neutrinos[0].GetX(), neutrinos[0].GetY(), neutrinos[0].GetZ());
                    has_vertex = true;
                    if (debugMode) {
                        LogInfo << "Event " << sg_event_id << " vertex at ("
            ...
            if (debugMode) LogInfo << " merged_tps has size " << merged_tps.size() << std::endl;
            ...
            if (bkg_event_id >= 0 && !bkg_tps.empty()) {
                ...
                for (const auto& bkg_tp : bkg_tps) {
                    ...
                    tp.SetEvent(sg_event);
                    tp.SetRun(sg_run); // HMS changed to match the signal event
                ...
                if (verboseMode) {
                    LogInfo << "Signal event " << sg_event_id << ": " << signal_tps.size() << " signal TPs + "
            ...
            if (debugMode) LogInfo << "made new event of size " << merged_tps.size() << std::endl;
            ...
            //tps_writer.WriteSingleEvent(merged_tps);
            //merged_tps_vec.push_back(merged_tps);
            // merged_true_vec.push_back(merged_true);
            // merged_nu_vec.push_back(merged_nu);
            if (debugMode)LogInfo << " about to write tps out "  << std::endl;
            tps_writer.WriteSingleEvent(merged_tps);  
            merged_tps.clear(); 
            bkg_tps.clear();     
            //if (debugMode)LogInfo << " after write out to " << output_tps_tree->GetName() << std::endl;

            //merged_tps.clear();
            signal_tp_counts.push_back(signal_tps.size());
            total_tps_count += signal_tps.size();
            signal_tps_by_event.clear();
